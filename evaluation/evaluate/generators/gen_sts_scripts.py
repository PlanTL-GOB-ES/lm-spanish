        ...
        for weight in weight_dec:
            for lr in learn_rate:
                with open(f"{model}_sts_batch{batch}_lr{lr}_decay{weight}.sh", "w") as f:
                    f.write(f"""#!/bin/bash
#SBATCH --job-name="{model}_sts_batch{batch}_lr{lr}_decay{weight}"
#SBATCH -D .
#SBATCH --output=../logs/{model}_sts_batch{batch}_lr{lr}_decay{weight}_%j.out
#SBATCH --error=../logs/{model}_sts_batch{batch}_lr{lr}_decay{weight}_%j.err
#SBATCH --ntasks=1
#SBATCH --gres gpu:2
...
python ../run_glue.py --model_name_or_path $MODEL --seed $SEED {bl}
                                          --dataset_script_path ../scripts/sts_dataset.py {bl}
                                          ...
                                          --metric_for_best_model combined_score --evaluation_strategy epoch --load_best_model_at_end
                """)
