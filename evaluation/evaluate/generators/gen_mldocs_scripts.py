...
bl = "\\" 
dir_name = "DIR_NAME='mldocs'_${BATCH_SIZE_PER_GPU}_${WEIGHT_DECAY}_${LEARN_RATE}_$(date +'%m-%d-%y_%H-%M')"
    ...
    for batch in batches:
        ...
        for weight in weight_dec:
            for lr in learn_rate:
                with open(f"{model}_mldocs_batch{batch}_lr{lr}_decay{weight}.sh", "w") as f:
                    f.write(f"""#!/bin/bash
#SBATCH --job-name="{model}_mldocs_batch{batch}_lr{lr}_decay{weight}"
#SBATCH -D .
#SBATCH --output=../logs/{model}_mldocs_batch{batch}_lr{lr}_decay{weight}_%j.out
#SBATCH --error=../logs/{model}_mldocs_batch{batch}_lr{lr}_decay{weight}_%j.err
#SBATCH --ntasks=1
#SBATCH --gres gpu:2
...
python ../run_glue.py --model_name_or_path $MODEL --seed $SEED {bl}
                                          --dataset_script_path ../scripts/mldoc_dataset.py {bl}
