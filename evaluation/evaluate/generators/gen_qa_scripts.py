...
bl = "\\" 
dir_name = "DIR_NAME='qa'_${BATCH_SIZE_PER_GPU}_${WEIGHT_DECAY}_${LEARN_RATE}_$(date +'%m-%d-%y_%H-%M')"
    ...
    for batch in batches:
        ...
        for weight in weight_dec:
            for lr in learn_rate:
                with open(f"{model}_qaes_batch{batch}_lr{lr}_decay{weight}.sh", "w") as f:
                    f.write(f"""#!/bin/bash
#SBATCH --job-name="{model}_qa_batch{batch}_lr{lr}_decay{weight}"
#SBATCH -D .
#SBATCH --output=../logs/{model}_qa_batch{batch}_lr{lr}_decay{weight}_%j.out
#SBATCH --error=../logs/{model}_qa_batch{batch}_lr{lr}_decay{weight}_%j.err
#SBATCH --ntasks=1
#SBATCH --gres gpu:2
...
python ../run_qa.py  --model_name_or_path $MODEL --seed $SEED {bl}
                                         --dataset_name '../scripts/viquiquad.py' {bl}
                                         ...
                                         --learning_rate $LEARN_RATE --warmup_ratio $WARMUP --weight_decay $WEIGHT_DECAY {bl}
...
python ../run_qa.py  --model_name_or_path $OUTPUT_DIR/$DIR_NAME --seed $SEED {bl}
                                         --dataset_name '../scripts/xquad-es.py' {bl}
                                         --do_eval {bl}
                                         --num_train_epochs $NUM_EPOCHS --gradient_accumulation_steps $GRADIENT_ACC_STEPS --per_device_train_batch_size $BATCH_SIZE --max_seq_length $MAX_SEQ_LENGTH {bl}
                                         --learning_rate $LEARN_RATE --warmup_ratio $WARMUP --weight_decay $WEIGHT_DECAY {bl}
                                         --output_dir $OUTPUT_DIR/$DIR_NAME/XQUADES --overwrite_output_dir {bl}
                                         --logging_dir $LOGGING_DIR/$DIR_NAME --logging_strategy epoch {bl}
                                         --cache_dir $CACHE_DIR/$DIR_NAME --overwrite_cache {bl}
                                         --metric_for_best_model f1 --evaluation_strategy epoch --load_best_model_at_end
                """)
