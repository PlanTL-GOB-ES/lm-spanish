        ...
        for weight in weight_dec:
            for lr in learn_rate:
                with open(f"{model}_xnli_batch{batch}_lr{lr}_decay{weight}.sh", "w") as f:
                    f.write(f"""#!/bin/bash
#SBATCH --job-name="{model}_xnli_batch{batch}_lr{lr}_decay{weight}"
#SBATCH -D .
#SBATCH --output=../logs/{model}_xnli_batch{batch}_lr{lr}_decay{weight}_%j.out
#SBATCH --error=../logs/{model}_xnli_batch{batch}_lr{lr}_decay{weight}_%j.err
#SBATCH --ntasks=1
#SBATCH --gres gpu:2
...
python ../run_glue.py --model_name_or_path $MODEL --seed $SEED {bl}
                                         --dataset_script_path ../scripts/xnli.py --dataset_config_name es {bl}
